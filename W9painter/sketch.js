...
function mapX(px) {
  return map(px, 0, 1, width, 0);
}
function mapY(py) {
  return map(py, 0, 1, 0, height);
}
  ...
  if (handsfree.data.facemesh) {
    if (handsfree.data.facemesh.multiFaceLandmarks) {
      ...
      if (nFaces > 0){
        ...
        var nose = faceLandmarks[whichFace][4];
        fill(255, 0, 0, 255);
        noseX = mapX(nose.x);
        noseY = mapY(nose.y); 
        circle(noseX, noseY, 30);

        paintDots.push({ x: noseX, y: noseY });
...
function startExplode() {
  ...
  centerX /= paintDots.length;
  centerY /= paintDots.length;
